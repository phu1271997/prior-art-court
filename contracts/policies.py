...
POLICIES: dict[str, str] = {
    ...
    "sla-clause": (
        "This category covers disputes over whether a written service-level "
        "agreement clause was tripped. Exhibits are (A) the SLA text itself "
        "and (B) the incident record (a status page snapshot, an incident "
        "report, or a monitoring dashboard export). Verdict INFRINGING means "
        "the clause WAS breached: the observed metric crossed the stated "
        "threshold for the stated duration and no listed exclusion applies. "
        "DERIVATIVE_FAIR means an exclusion (planned maintenance, force "
        "majeure, customer misuse) is documented and covers the incident. "
        "INDEPENDENT means the metric never crossed the threshold or the "
        "clause does not apply to the metric measured. Apply the SLA's own "
        "definitions of 'downtime', 'availability', 'incident', and "
        "'measured region'. If the SLA gives a formula, apply it — do not "
        "substitute a general one. If a term is ambiguous, favour the "
        "reading closer to the customer's ordinary understanding on the "
        "day the SLA was signed; a vendor that wanted a narrower reading "
        "had the drafting pen."
    ),
    "academic-misconduct": (
        "This category covers plagiarism, undisclosed authorship, and "
        "duplicate submission complaints against academic work. Exhibits are "
        "(A) the accused paper/thesis/preprint and (B) the alleged source. "
        "Protected expression is prose, argument structure, chosen data, "
        "figures, and specific formulations. NOT PROTECTED: field-standard "
        "notation, replicated results, standard methods, well-known "
        "background, and any material B itself cites. Verdict INFRINGING "
        "requires reproduction beyond what a citation would license: "
        "substantial verbatim or lightly paraphrased passages without "
        "citation, or with a citation so understated that a reader would "
        "not know they were reading B. Verdict DERIVATIVE_FAIR covers "
        "legitimate quotation, replication with attribution, and building "
        "on prior work with proper cite. Self-plagiarism between a "
        "preprint and its final version by the same authors is not "
        "misconduct; presentation of another author's data/figures without "
        "attribution IS. Duplicate submission (same paper published twice "
        "in two venues without cross-reference) is INFRINGING."
    ),
    "bug-bounty-severity": (
        "This category covers disputes about the assigned severity of a "
        "reported security vulnerability. Exhibits are (A) the vendor's "
        "published severity rubric and (B) the advisory / proof-of-concept "
        "the reporter submitted. Verdict INFRINGING means the assigned "
        "severity is UNDERstated — the advisory clearly demonstrates the "
        "impact class the rubric reserves for a higher tier. Verdict "
        "DERIVATIVE_FAIR means the assigned severity is defensible under "
        "the rubric even if a reader might reasonably disagree; treat as "
        "such where the rubric is vague on the specific behavioural class "
        "demonstrated. Verdict INDEPENDENT means the assigned severity is "
        "correct or OVERstated (the reporter is asking for a higher tier "
        "than the rubric actually permits). Apply the rubric literally. "
        "Do not import CVSS or another rubric unless the vendor's rubric "
        "explicitly does so. A vulnerability that combines two mid-tier "
        "primitives into a higher-tier chain is judged on the chain, not "
        "the parts — the demonstrated impact is what matters."
    ),
    "patent-claim": (
