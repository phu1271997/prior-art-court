...
POLICIES: dict[str, str] = {
    ...
    "patent-claim": (
        "This category covers patent-claim disputes: the exhibits are (A) a "
        "patent-claim document — a granted claim, a pending application, or an "
        "informal claim of invention — and (B) an alleged prior art or accused "
        "practice. Adjudicate under the two-question framework U.S. patent law "
        "uses because it is the clearest doctrine, but apply the substance, not "
        "the labels — the court is not U.S.-jurisdictional, it is doctrine-"
        "faithful.\n\n"
        "QUESTION ONE — ANTICIPATION (§102 test): does exhibit B disclose EVERY "
        "element of A's independent claim, arranged as claimed, in a single "
        "reference? Missing even one element defeats anticipation. If B "
        "anticipates A, then B is prior art and A's claim is not novel; verdict "
        "INFRINGING (the complaint that A is the original is inverted).\n\n"
        "QUESTION TWO — OBVIOUSNESS (§103 test): if B does NOT anticipate but a "
        "person of ordinary skill in the art, given B plus the state of the art "
        "the exhibits themselves reference, would have found A's combination "
        "obvious, verdict DERIVATIVE_FAIR. Weigh the Graham factors: scope and "
        "content of B, differences between B and A, level of ordinary skill, "
        "and any objective indicia (long-felt need, commercial success, "
        "unexpected results) visible in the exhibits.\n\n"
        "PROTECTED (as expression): the SPECIFIC combination of claim elements "
        "and the way they interact. NOT PROTECTED: individual elements known in "
        "the art, the problem being solved, and functional descriptions of a "
        "result. Two independent inventors solving the same problem with the "
        "same well-known elements is convergence, not copying.\n\n"
        "Return INDEPENDENT only where B fails BOTH tests: it does not "
        "anticipate, and the modification from B to A is non-obvious. The "
        "first_publisher field decides precedence on appeal; a work that "
        "predates the alleged original is prior art regardless of similarity."
    ),
}
